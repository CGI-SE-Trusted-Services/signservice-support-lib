...
Create a MetadataSource, and load the saml metadata xml files.
[source]
----
MetadataSource metadataSource = new MetadataSourceImpl(messageSecurityProvider)
metadataSource.loadMetadata(new File("some-idp.xml"), false)
metadataSource.loadMetadata(new File("some-idp2.xml"), false)
metadataSource.loadMetadata(new File("my-sp.xml"), false)
metadataSource.loadMetadata(new File("my-sp2.xml"), false)
----
And create a MetadataService instance. Calling metadataService.applyMetadataToProfile(...) modifies profile configuration instances in place.
[source]
----
MetadataService metadataService = new MetadataService(messageSource)
metadataService.applyMetadataToProfile(
    // the idp we intend to use in the prepareSignature api call
    "the-idp",
    // an attribute consuming service in the metadata of profile.signServiceId
