...
The library is configured and initialized using builder pattern.
The following gives a basic example of how to build an instance of the API:
...
When creating an instance of the support service API a message security provider must be provided.
The message security provider is used when signing signature requests and when verifying signature responses.
A custom message security provider can be created by implementing `se.signatureservice.messages.MessageSecurityProvider`.
It is also possible to use a build-in simple message security provider.
The following gives an example of how to create a simple message security provider:
...
An auto-submit form can be used in order to re-direct the user browser with the signature request.
This form can be generated using a helper:

    String formContent = SupportLibraryUtils.generateRedirectHtml(preparedSignature);

After a successful signature flow the signature response will be sent to the consumer URL.
The library can then be used to create the complete signed document(s), shown through the following example:
...
The API can be used in order to verify a signed document and to assure that the document has not been altered.
The following example shows how to verify a signed document using the API:
...
In this case, instead of sending the full document, the client provides a `DocumentRef` object with a `referenceId`.
The actual content is retrieved later using a `DocumentResolver` implementation.
...
When using the API a profile configuration instance (`se.signatureservice.support.system.SupportAPIProfile`) needs to be created and provided.
The profile configuration contains a lot of settings to control how the request is created and how the signature is performed.
Profile configuration is created using builder pattern.
The following gives a basic example of how to create a profile configuration:
...
SupportAPIProfile profileConfig = new SupportAPIProfile.Builder()
    ...
    // This allows requested attributes found in metadata, to overwrite those already in the profile.
    .fetchCertAttributesFromMetaData(true)

    // This allows authNContextClassRefs found in the metadata to overwrite those already in the profileConfig.
...
=== Using metadata to modify profiles

Profiles can be automatically enriched with information extracted from SAML metadata.
This is useful when working with IdPs (Identity Providers) and SPs (Service Providers),
since entity descriptors often include display names, authentication context references, certificate attributes, and user ID mappings.

It is possible to modify profile configuration instances, using entity metadata for Idp:s and SP:s.

==== 1. Create and load a MetadataSource

The `MetadataSource` is responsible for reading and storing metadata from SAML XML files (or raw XML bytes).

[source,java]
----
MetadataSource metadataSource = new MetadataSourceImpl(messageSecurityProvider);

// Load metadata from files (no signature verification by default)
metadataSource.loadMetadata(new File("some-idp.xml"));
metadataSource.loadMetadata(new File("my-sp.xml"));
----

NOTE: Signature verification is disabled by default.
If you want to verify SAML XML signatures, use one of the overloads with the `verifySignature` parameter:

[source,java]
----
// From File
metadataSource.loadMetadata(new File("some-idp.xml"), true);

// From raw bytes
byte[] xmlBytes = Files.readAllBytes(Path.of("some-idp.xml"));
metadataSource.loadMetadata(xmlBytes, true);
----

Available overloads:

* `loadMetadata(File file)`
* `loadMetadata(File file, boolean verifySignature)`
* `loadMetadata(byte[] bytes)`
* `loadMetadata(byte[] bytes, boolean verifySignature)`

==== 2. Apply metadata to a profile

Once a `MetadataSource` has been populated, you can use `MetadataService` to modify profile configuration instances in place.

[source,groovy]
----
MetadataService metadataService = new MetadataService(messageSource)

metadataService.applyMetadataToProfile(
    // The IdP to be used in the prepareSignature API call
    "the-idp",

    // An attribute consuming service name from the metadata of profile.signServiceId
    "some-attr-consuming-service-name",

    // Preferred display language
    "en",

    // The profile configuration instance that will be modified
    profileConfig,

    // Metadata source containing IdP/SP metadata
    metadataSource
)
----
...
When using a profile that has been configured with a -T, -LT or -LTA signature level, a time stamp will be requested from a time stamp server.
This applies for the following signature levels:
...
Time stamp configuration is performed by specifying `timeStamp` when building a profile _(Only relevant parts are included in the example)_:
...
The following table shows available settings that can be used.
Each field within the table is specified using its setter.
...
It consists of an image and text element that is embedded into the document after it has been signed.
The purpose is to give a visual indication that the document is digitally signed,
and some information about the signature such as the name of the person that signed the document, the signature time.

IMPORTANT: A visible signature does not replace the actual cryptographic signature within a digitally signed document.
It should not be used as a way of verifying that a document has been signed, as it is easy to forge.
...
within the template must be specified using curly-bracket syntax, ex. `+{signerName}+`, and is
case-sensitive.The following gives an example of a signature text template.
...
There are a number of signature parameters that can be used in order to control visible signatures.
These attributes are specified in the initial call to `prepareSignature(...)` and can be specified either for all documents or for individual documents.
The following table shows available signature attributes related to visible signatures.
...
| validationPolicy(String validationPolicy) [[validation-policy-property]]
...
- `1` = `NO_CHANGE_PERMITTED`
No changes are permitted after signing.
Any modification will invalidate the signature.

- `2` = `MINIMAL_CHANGES_PERMITTED`
Only form filling, instantiating page templates, and signing are allowed.
All other modifications invalidate the signature.

- `3` = `CHANGES_PERMITTED`
In addition to the permissions in level 2, this allows annotation creation, deletion, and modification.
Other changes still invalidate the signature.
...
====== DocMDP Validation Policy Constraint [[doc-mdp]]
...
in addition to the mandatory userId attribute (see defaultUserIdAttributeMapping) that is always included as a signer attribute.
...
The configuration entry `trustedAuthenticationServices`, when specified, contains all the authentication services
that are trusted by the support service, and also the explicit configuration to be used for each individual
service which will override any default configuration shared between all authentication services.
Otherwise, when configuration entry `trustedAuthenticationServices` is not specified, all metadata parsed via
`MetadataSource` are trusted.

NOTE: defaultDisplayName will be used if display name is not available in metadata.

Example of profile trustedAuthenticationServices configuration:
[source,groovy]
----
def profile = new SupportAPIProfile.Builder()
        .trustedAuthenticationServices([
            iDPTest: [
                entityId: "https://idptest.service.se/samlv2/idp/metadata",
...
When `trustedAuthenticationServices` is not specified the _entityId_ and _defaultDisplayName_ is automatically parsed from
each EntityDescriptor element which contains IDPSSODescriptors, of provided metadata (loaded via MetadataSource).

* _entityId_:
Extracted from the _entityID_ attribute value of the EntityDescriptor element.

* _defaultDisplayName_ (extracted in order of priority (highest first), based on availability):
1. Extracted from _DisplayName_ of the metadata extension namespace in IDPSSODescriptor.
2. Extracted from _OrganizationDisplayName_ in _Organization_ of the EntityDescriptor element.
3. Extracted from the Common-Name(CN) of the X509 Certificate in _KeyDescriptor_, with attribute "signing", of the IDPSSODescriptor element.

A Preferred language code, _preferredLang_ variable, is used when extracting _defaultDisplayName_ XML element data, as above in point 1 and 2.
Here the attribute is used to indicate the language of the text content within an element.
If the variable is NOT set, it received from the language code of the system's default locale.

Order of which content to extract based set _Preferred language_ and matched with the language attribute value of the specific XML element:

1. The set Preferred language
2. "en"
3. The first one of the XML elements

===== Configuration value: requestedCertAttributes

*Manual Configuration:*

The `requestedCertAttributes` configuration within a profile defines which subject attributes should be included in the signer’s certificate,
and thereby associated with the signer of the generated signature as a result of the sign request.
...
For the `serialNumber` attribute, the `samlAttributeName` can be configured either as a single string or as a list of maps.
By default, the order is set to 0; attributes with order value 1 can be filtered out in the sign response.
...
*Automatic Population from Metadata:*

To automatically populate the `requestedCertAttributes` section of a profile from SAML metadata (loaded via `MetadataSource`),
enable the fetchCertAttributesFromMetaData option in the profile.
...
Once this flag is set to true, all RequestedAttribute elements within each AttributeConsumingService that matches the configured serviceName parameter will be parsed.
This parsing is performed for each EntityDescriptor element that contains SPSSODescriptors.

If any requestedCertAttributes were successfully parsed from the provided metadata, the attributes specified in the profile configuration will be overridden.
However, if no attributes could be automatically parsed, the system will fall back to the requestedCertAttributes values provided in the configuration, if set.

NOTE: Again, this parsing is only performed for each AttributeConsumingService that matched the configured serviceName parameter.

Below follows an example of when 2 RequestedAttribute are parsed from given metadata:
...
new MetadataService(messageSource).applyMetadataToProfile(null, "test_service_name", "en", profile, metadataSource)
----

*Input* XML Metadata containing AttributeConsumingService elements:
[source,xml]
----
<md:AttributeConsumingService index="0" isDefault="true">
    <md:ServiceName xml:lang="sv">test_service_name</md:ServiceName>
    <md:RequestedAttribute Name="urn:oid:1.2.752.29.4.13" FriendlyName="serialNumber"/>
    <md:RequestedAttribute Name="urn:oid:2.5.4.42" isRequired="true"/>
</md:AttributeConsumingService>
----

*Output* requestedCertAttributes as stored in SupportAPIProfile after parsing metadata:
[source,yaml]
----
requestedCertAttributes:
  serialNumber:
...
====== Custom Attribute Mapping: metadataCustomCertAttribute

In cases where the `Name` in a `RequestedAttribute` from the metadata does **not** conform to the standard mapping table,
a **custom attribute mapping** can be provided via `metadataCustomCertAttribute`, identical to how `requestedCertAttributes` is configured.
This mapping defines how each SAML attribute from metadata should be converted into `requestedCertAttributes`.

Configuration Keys:

- _samlAttributeName_: The SAML attribute `Name` to match in metadata. Can be a single string or a list of strings.
- _certAttributeRef_: The certificate attribute reference to map the `samlAttributeName` to.
- _certNameType_ (optional): The certificate name type for this attribute.
- _required_ (optional): Overrides the `isRequired` value from the metadata. If not provided, the system uses `isRequired` from metadata, if available.

Rules for `FriendlyName`:

- If a `RequestedAttribute` has a `FriendlyName`, it is used as the key in the resulting `requestedCertAttributes`.
- If no `FriendlyName` is provided, the system attempts to derive a key from the standard mapping table.

_Example with one samlAttributeName configured:_

*Input* configuration as stored in SupportAPIProfile:
[source,groovy]
----
def profile = new SupportAPIProfile.Builder()
        .fetchCertAttributesFromMetaData(true)
        .metadataCustomCertAttribute(["givenName": ["samlAttributeName":"http://sambi.se/attributes/1/givenName[attrName]", "certAttributeRef":"2.5.4.42"]])
...
<md:AttributeConsumingService index="0" isDefault="true">
    <md:ServiceName xml:lang="sv">test_service_name</md:ServiceName>
    <md:RequestedAttribute Name="http://sambi.se/attributes/1/givenName" FriendlyName="serialNumber"/>
...
_Example with multiple samlAttributeName configured:_
...
def profile = new SupportAPIProfile.Builder()
        .fetchCertAttributesFromMetaData(true)
        .metadataCustomCertAttribute(["surName": ["samlAttributeName":["http://sambi.se/attributes/1/givenName[attrName]", "urn:surname"], "certAttributeRef":"2.5.4.4", "certNameType": "sda"]])
...
<md:AttributeConsumingService index="0" isDefault="true">
    <md:ServiceName xml:lang="sv">test_service_name</md:ServiceName>
    <md:RequestedAttribute Name="urn:surname"/>
    <md:RequestedAttribute Name="urn:oid:2.5.4.42" isRequired="true"/>
</md:AttributeConsumingService>
----
...
There are two main ways to decide which authnContextClassRefs will be set for a sign request

. Via the selected profileConfig, and in this case, optionally, authnContextClassRefs can be read from metadata too. See section <<refs-from-profiles>>
. In each call to SignSupportV2Endpoint.PrepareSignature. This will override any profile- or metadata settings. See section <<refs-in-api-call>>

[[refs-from-profiles]]
====== defaultAuthnContextClassRefs from profiles or metadata

*Manual Configuration:*

The most straightforward method is to define `defaultAuthnContextClassRefs` directly in the profile.
These values will be used in generated AuthnRequests sent to IDPs during a SignRequest, without requiring a SignMessage.
...
These defaults can be overridden per trusted authentication service using the `authnContextClassRefs` setting.
...
To minimize manual configuration, you can automatically populate the `defaultAuthnContextClassRefs` section from SAML metadata.
This requires enabling the `fetchAuthnContextClassRefFromMetaData` option in the profile.
When enabled, values extracted from metadata take precedence over any profile-specific defaults.
...
new MetadataService(messageSource).applyMetadataToProfile("authenticationServiceId", null, "en", profile, metadataSource)
----

With `fetchAuthnContextClassRefFromMetaData` enabled, `AttributeValue` elements in `Extensions` and `EntityAttributes`
that belong to the `Attribute` named _'urn:oasis:names:tc:SAML:attribute:assurance-certification'_ are automatically parsed.
These values are then applied to the `authnContextClassRefs` for trusted authentication services whose `entityId` matches the parsed metadata.

You can also explicitly specify an `AuthnContextClassRef` in a SOAP signature request using the `signatureAttributes` parameter.
The key `auth_context_class_ref` allows setting a specific Authentication Context Class Reference.

- If the reference is **set**, it is validated against the allowed `authnContextClassRefs` for the given authentication service.
- If the reference is **not found**, an error is thrown indicating an invalid authentication context class reference.
- If the reference is **not set**, all available `authnContextClassRefs` for the authentication service are used by default.

NOTE: Assigning new `authnContextClassRefs` will **overwrite** any existing values for the corresponding trusted authentication service's IDP.

Example: If two `authnContextClassRefs` are parsed from metadata, they will be applied to the matched `entityId` as shown below.
...
def authenticationServiceId = "https://idp.cgi.com/v2/metadata"

new MetadataService(messageSource).applyMetadataToProfile(authenticationServiceId, null, null, profile, metadataSource)
----

*Input* XML Metadata
[source,xml]
----
<?xml version="1.0" encoding="UTF-8"?><md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" entityID="https://idp.cgi.com/v2/metadata" ID="_07c71e61c3100416e3eff9e8c69c3d49e7">
  <md:Extensions>
...
*Output* authnContextClassRefs as stored in SupportAPIProfile after parsing metadata:
[source,yaml]
----
trustedAuthenticationServices:
  idpTest1:
    entityId: "https://idp.cgi.com/v2/metadata"
    authnContextClassRefs: ["Ref:A", "Ref:B"]
----

[[refs-in-api-call]]
====== Overriding AuthnContextClassRefs in SignSupportV2Endpoint.PrepareSignature calls

The `SignSupportV2Endpoint.prepareSignature` method includes an **optional** `authnContextClassRefs` parameter:
...
If provided, the `authnContextClassRefs` list **overrides any profile- or metadata-defined settings** for that API call.
This enables fully dynamic use of `authnContextClassRefs` alongside static configuration.

NOTE: Any `signatureAttributes` (described above) are still applied as usual, but they are now validated against the API-supplied parameters instead of the profile configuration.
In this context, the use-case for `signatureAttributes` is reduced, since the API parameters provide more flexibility and control.

===== Configuration value: DefaultUserIdAttributeMapping

This section explains how to automatically parse the `DefaultUserIdAttributeMapping` from the metadata.
This eliminates the need for manual specification of the SAML Attribute name for user ID mapping.

*Manual Specification of SAML Attribute:* +
If automatic parsing is not used, the SAML Attribute name must be specified manually. This can be done through: +
- Profile-specific setting `DefaultUserIdAttributeMapping` or `defaultUserIdAttributeMappingValues` +
- Authentication service configuration `userIdAttributeMapping` +

*Via the profile-specific setting:*
[source, groovy]
----
def profile = new SupportAPIProfile.Builder()
...
----
def profile = new SupportAPIProfile.Builder()
                .defaultUserIdAttributeMappingValues(["urn:oid:1.2.752.201.3.4","urn:oid:1.2.752.29.4.13"])
----

*Via the profile-specific authentication service configuration:*
...
*Enabling Automatic Parsing:* +

To enable automatic parsing, ensure that the profile-specific `defaultUserIdAttributeMapping` / `userIdAttributeMapping`*(deprecated)* and
authentication service configuration `userIdAttributeMapping` settings are not set.

How It Works: +
1. The process first looks for a `ServiceName` attribute in each `AttributeConsumingService` that matches the required set parameter `ServiceName`. +
2. It then checks each `RequestedAttribute` in the `AttributeConsumingService` for a match in `Name` with the values in the default UserId Attribute Mapping List. +
3. When a matching value for `Name` is found it sets this value for the `DefaultUserIdAttributeMapping`.

NOTE: If multiple or no matches are found an error thrown.

Default UserId Attribute Mapping List: +
[source,text]
----
...
These default UserId Attribute Mapping values can be overridden via the profile-specific configuration
`DefaultUserIdAttributeMapping` (String) or `defaultUserIdAttributeMappingValues` (List of Strings).

Example of the output configuration when using the _defaultUserIdAttributeMappingValues_ setting: +

*Input* configuration as stored in SupportAPIProfile:
[source, groovy]
----
def profileConfig = new SupportAPIProfile.Builder()
                .defaultUserIdAttributeMappingValues(["urn:name", "http://sambi.se/attributes/1/id"])

new MetadataService(messageSource).applyMetadataToProfile(null, "service1", "en", profileConfig, metadataSource)
----
...
----
    <md:AttributeConsumingService index="0">
      <md:ServiceName xml:lang="sv">service1</md:ServiceName>
      <md:RequestedAttribute Name="urn:givenName"/>
      <md:RequestedAttribute Name="http://sambi.se/attributes/1/id"/>
    </md:AttributeConsumingService>
----

*Output* DefaultUserIdAttributeMapping as stored in SupportAPIProfile after parsing metadata:
[source,yaml]
----
    DefaultUserIdAttributeMapping: "http://sambi.se/attributes/1/id"
----

Example of the output configuration when parsing via the default UserId Attribute Mapping List: +

*Input*:
[source, groovy]
----
new MetadataService(messageSource).applyMetadataToProfile(null, "service1", "en", SupportAPIProfile, metadataSource)
----
...
----
    <md:AttributeConsumingService index="0">
      <md:ServiceName xml:lang="sv">service1</md:ServiceName>
      <md:RequestedAttribute Name="urn:givenName"/>
      <md:RequestedAttribute Name="urn:oid:1.2.752.29.4.13"/>
    </md:AttributeConsumingService>
----
...
The eu.europa.ec.joinup.sd-dss library ships with a default validation policy, used for
validating signed documents. The same file, unaltered, is provided with this library, as a possible starting point for customizations.

See also:

* A typical customization example <<doc-mdp>>
* The api profile configuration property used to point out the validation policy <<validation-policy-property>>

== Troubleshooting

=== Error w Signing Document: Unable to Save Document
