...
* Bump Tomcat dependency used in testing from 10.1.39 to 10.1.41
* Bump Spock Framework Core Module to 2.4-M6-groovy-3.0
* Removed unnecessary com.squareup.okhttp3:okhttp and org.json:json dependency.
...
* Added support for overriding authnContextClassRefs settings in signservice-support-lib SupportServiceAPI.prepareSignature calls
    ...
    will override any profile settings for 'authnContextClassRefs', for that API call. As usual, the values must map to settings in the cental system.
